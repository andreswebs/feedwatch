...
var schemaRegistry = map[string]cmdMeta{
	...
	// which the reflector would misrender as an array of bytes, so a reflected
	// oneOf would not describe the envelope cleanly (ADR 0005).
	"schema": {exitCodes: defaultExitCodes(), output: jsonschema.Scalar("object", "a Schema when narrowed to one command, otherwise the SchemaResult envelope: {tool, version, commands, exit_codes, errors, global_flags} atop the schema head")},
}

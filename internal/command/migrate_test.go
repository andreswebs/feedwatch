...
func TestMigrateStatusFreshDB(t *testing.T) {
	res := runCLI(t, "1.2.3", "feedwatch", "--db", tempDB(t), "migrate", "--status")
...
func TestMigrateStatusIdempotent(t *testing.T) {
	...
	first := runCLI(t, "1.2.3", "feedwatch", "--db", db, "migrate", "--status")
	second := runCLI(t, "1.2.3", "feedwatch", "--db", db, "migrate", "--status")
...
// TestMigrateUnwritableDBExits69 covers walking-skeleton behavior 4: a --db whose
// parent directory does not exist cannot be opened, so the store-unavailable
// failure maps to exit 69 (EX_UNAVAILABLE) with a JSON error object on stderr
// and no stdout.
func TestMigrateUnwritableDBExits69(t *testing.T) {
	bad := filepath.Join(t.TempDir(), "missing-dir", "feedwatch.db")
	res := runCLI(t, "1.2.3", "feedwatch", "--db", bad, "migrate", "--status")

	if res.code != 69 {
		t.Errorf("exit code = %d, want 69 (store unavailable)", res.code)
	}
	if res.out != "" {
...
func TestMigrateAppliesThenStatusClean(t *testing.T) {
	...
	res := runCLI(t, "1.2.3", "feedwatch", "--db", db, "migrate")
	if res.code != 0 {

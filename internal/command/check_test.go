...
// TestCheckUnknownRefExits64 covers: an unknown named ref is a usage error, exit 64.
func TestCheckUnknownRefExits64(t *testing.T) {
	st, fetcher, parser := newCheckDoubles(t)

	res := runCheck(t, st, fetcher, parser, "check", "https://no-such.example/feed.xml")

	if res.code != 64 {
		t.Errorf("exit code = %d, want 64 (usage) for unknown ref", res.code)
	}
}

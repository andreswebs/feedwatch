...
// TestRegisteredCodesDeclaredInTables is the reciprocal of
// TestExitCodeTablesCoverExitCodeFor: every code in terr.All() must map to an
// exit code the command tables declare. The first test proves the tables cover
// what ExitCodeFor produces; this one proves they cover what the error registry
// registers, so the documented error surface and the exit-code tables cannot
// drift apart in either direction (ADR 0001/0002).
func TestRegisteredCodesDeclaredInTables(t *testing.T) {
	registered := terr.All()
	if len(registered) == 0 {
		t.Fatal("terr.All() is empty; the error registry should carry the coded sentinels")
	}

	// defaultExitCodes underlies every command; poll and check only extend it
	// with result sub-codes, so a code declared in default is declared everywhere.
	def := defaultExitCodes()
	for _, e := range registered {
		code := strconv.Itoa(e.ExitCode())
		if _, ok := def[code]; !ok {
			t.Errorf("registered error %q maps to exit %s, not declared in the command tables: %v", e.Code(), code, def)
		}
	}
}

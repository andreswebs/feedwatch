...
func TestItemsFieldsUnknownRejected(t *testing.T) {
	...
	for _, arg := range []string{"bogusfield", "title,bogus"} {
		...
		if res.code != 64 {
			t.Errorf("--fields %q should exit 64 (usage), got code=%d", arg, res.code)
		}
		if res.out != "" {
...
func TestItemsFieldsDidYouMean(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			res := runItems(t, st, clk, "--fields", tc.arg)
			if res.code != 64 {
				t.Fatalf("--fields %q should exit 64 (usage), got code=%d", tc.arg, res.code)
			}
			if res.out != "" {
...
func TestItemsInvalidOrder(t *testing.T) {
	...
	if res.code != 64 {
		t.Errorf("invalid --order should exit 64 (usage), got code=%d", res.code)
	}
	if res.out != "" {
...
func TestItemsTimeFieldInvalid(t *testing.T) {
	...
	if res.code != 64 {
		t.Errorf("invalid --time-field should exit 64 (usage), got code=%d", res.code)
	}
	if res.out != "" {
		t.Errorf("stdout should be empty on usage error, got %q", res.out)
	}
...
func TestItemsFieldsErrorListsValidFields(t *testing.T) {
	...
	if res.code != 64 {
		t.Fatalf("--fields published should exit 64 (usage), got code=%d", res.code)
	}
	msg := errorMessage(t, res.err)

...
// usage error, exit 64, with a message pointing to discover, and is not stored.
func TestAddHTMLPageIsRejectedPointingToDiscover(t *testing.T) {
	...
	if !res.exited || res.code != 64 {
		t.Fatalf("rejecting a non-feed should exit 64 (usage), got exited=%v code=%d", res.exited, res.code)
	}
	if res.out != "" {
...
// bare host (no scheme) is rejected as a usage error, exit 64, before any fetch.
func TestAddNonHTTPSchemeIsUsageError(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !res.exited || res.code != 64 {
				t.Fatalf("bad URL should exit 64 (usage), got exited=%v code=%d", res.exited, res.code)
			}
			var env errEnvelope
			if err := json.Unmarshal([]byte(res.err), &env); err != nil {
				t.Fatalf("stderr is not an error envelope: %v\ngot: %q", err, res.err)

...
// whether alone or alongside valid names, is rejected with exit 64 and empty
...
func TestItemsFieldsUnknownRejected(t *testing.T) {
	...
	for _, arg := range []string{"bogusfield", "title,bogus"} {
		...
		if !res.exited || res.code != 64 {
			t.Errorf("--fields %q should exit 64 (usage), got exited=%v code=%d", arg, res.exited, res.code)
		}
		if res.out != "" {
...
// TestItemsFieldsDidYouMean covers that a close typo in --fields still exits 64
// with empty stdout, but the usage error carries a did-you-mean suggestion;
// a name with no close match exits 64 with no suggestion (fee-n4p6).
func TestItemsFieldsDidYouMean(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			res := runItems(t, st, clk, "--fields", tc.arg)
			if !res.exited || res.code != 64 {
				t.Fatalf("--fields %q should exit 64 (usage), got exited=%v code=%d", tc.arg, res.exited, res.code)
			}
			if res.out != "" {
...
// whole-invocation usage failure (exit 64) with no result on stdout.
func TestItemsInvalidOrder(t *testing.T) {
	...
	if !res.exited || res.code != 64 {
		t.Errorf("invalid --order should exit 64 (usage), got exited=%v code=%d", res.exited, res.code)
	}
	if res.out != "" {
...
// value is a whole-invocation usage failure (exit 64) with empty stdout.
func TestItemsTimeFieldInvalid(t *testing.T) {
	...
	if !res.exited || res.code != 64 {
		t.Errorf("invalid --time-field should exit 64 (usage), got exited=%v code=%d", res.exited, res.code)
	}
	if res.out != "" {
		t.Errorf("stdout should be empty on usage error, got %q", res.out)
	}
...
func TestItemsFieldsErrorListsValidFields(t *testing.T) {
	...
	if !res.exited || res.code != 64 {
		t.Fatalf("--fields published should exit 64 (usage), got exited=%v code=%d", res.exited, res.code)
	}
	msg := errorMessage(t, res.err)

...
// renders it as a config error and exits 78 (EX_CONFIG).
...
// emitting the same usage error shape and exit code (64, EX_USAGE, per ADR
// 0001) as rootAction.
...
// usage-category JSON error on stderr and exits 64 (EX_USAGE, per ADR 0001).
...
// FeedError so the boundary renders it as structured JSON and exits 64 (EX_USAGE).

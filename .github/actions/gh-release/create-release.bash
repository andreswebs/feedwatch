...
```sh
gh attestation verify feedwatch-linux-amd64-*.tar.gz --repo @REPO@
```
...
```sh
cosign verify-blob \
  --bundle feedwatch-*.spdx.json.sigstore.json \
  --certificate-identity-regexp "https://github.com/@REPO@/.github/workflows/release.yml@refs/tags/.*" \
  --certificate-oidc-issuer "https://token.actions.githubusercontent.com" \
  feedwatch-*.spdx.json
```
...
gh release create "${TAG}" \
    ...
    "${DIST_DIR}/feedwatch-${TAG}.spdx.json" \
    "${DIST_DIR}/feedwatch-${TAG}.spdx.json.sigstore.json"
